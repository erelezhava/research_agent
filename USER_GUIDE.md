...
| **Completed — known limitations** | The useful bounded follow-up passes are complete, the citation check passed, and the report clearly records the remaining gaps | Read the report normally. There is no need to keep clicking Continue; start a new project only for a different scope or new source material. |
| **Completed with warnings** | A report was written, but a material uncertainty/budget limit remains, or the citation check found a problem or could not complete | The report is still shown in full. Follow the action shown: a budget-limited run offers **Continue research**, while a structural citation defect offers **Ask Claude to fix this** |
...
| **The planned research budget is exhausted before every check finishes** | The existing report stays available as **Completed with warnings** | Click **Continue research** to keep the same session, report, and evidence and run one additional bounded pass focused on unfinished checks. At most two such passes are allowed; after that the project becomes **Completed — known limitations** and stops offering another continuation. |
